...
mc_stats = mc_simple(num_trials=trials, sde_solver=solver, payoff=aon_payoff, discount=np.exp(-0.06), shared_noise=True)
mc_stats.print()
...
cv_solver = SdeSolver(sde=gbm_cv, time=3, num_steps=steps*5, dimension=2)
...
solver = SdeSolver(sde=gbm, time=3, num_steps=steps, dimension=1)
gbm_approx = GbmApproximator(basis=[basis_1, basis_2, basis_3], ts=ts, mu=gbm.mu, sigma=gbm.sigma)
new_cv_stats = mc_control_variate(num_trials=(1000, 10000), simple_solver=solver, approximator=gbm_approx,
                                  payoff=aon_payoff, discount=np.exp(-0.06))
new_cv_stats.print()
"""
